...
23. **Citations verified distinct; three cosmetic defects found and DEFERRED by the user (2026-08-11).** User asked for confirmation that each thesis's APA and MLA citations really are two different formats. **Verified against the live data: 19 pairs compared, 0 identical, and no two theses share a citation string.** The templates differ structurally in the way the two styles actually differ — APA puts the year in parentheses after the authors and leaves the title unquoted (`Authors (Year). Title [Unpublished thesis]. Mater Dei College.`), MLA quotes the title and moves the year to the end (`Authors. "Title." Unpublished thesis, Mater Dei College, Year.`). The claim in the documentation holds. Three defects surfaced during that check, all shown to the user, all left as-is on their instruction:
    - **Thesis #24 has TWO APA citations** (ids 3 and 9 — the generated one plus a staff-edited one). The design assumes exactly one row per `(thesis_id, format_type)` and `CitationController::generate()` relies on `firstOrCreate`, but **there is no unique index on that pair** — only plain foreign keys — so nothing at the database level prevents duplicates. Whichever row is read first wins, which is effectively arbitrary. A unique index plus de-duplication would fix it.
    - **Three citations have drifted from their thesis.** Staff edited them, then the title changed; nothing re-syncs an edited citation, by design (the edit is meant to survive). #24's APA still says *"Test Citation Test"* while the thesis is now *"Test Cover Image"*, and #20's reads only *"Test test"*.
    - **Two MLA citations show a double period** where an author name already ends in an initial: `ALAMPAYAN, WILSON JR. A.. "AQUATRACK…"`.
    - **Accuracy caveat worth knowing before the defense** (not a code bug): strict APA wants authors as *"Last, F. M."*, but `authors` is a free-text field, so whatever staff typed is what appears — "Kent Concha, Adam Esto" comes out first-name-first. APA 7 also expects the degree (*"[Unpublished master's thesis]"*) rather than the generic *"[Unpublished thesis]"*. The system formats faithfully; the input is what deviates. A panelist who knows APA would notice this before anything else.
...
- Self-service password change from the account's own Profile; admin-assisted password reset for any account (Staff/Super Admin only) — by deliberate design there is no self-service "forgot password" email flow. **The login page says so plainly**, directing anyone locked out to ask library staff, rather than offering a reset link that leads nowhere.
